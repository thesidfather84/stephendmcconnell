...
import { createPodcastAction, type ActionResult } from "@/lib/admin-content-actions";

const initialState: ActionResult = {};

export function PodcastForm() {
  const [state, formAction, pending] = useActionState(
    async (_prev: ActionResult, formData: FormData) => createPodcastAction(formData),
    initialState
  );
  ...
  return (
    <form action={formAction} className="space-y-6">
      ...
      <div>
        <label htmlFor="podcastUrl" className="block text-sm font-semibold text-navy">
          Podcast link
        </label>
        <input
          id="podcastUrl"
