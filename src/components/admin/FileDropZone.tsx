...
export function FileDropZone({
  name,
  accept,
  label = "Drag a file here, or click to choose one",
  hint = "Files up to 4MB.",
}: {
  name: string;
  accept?: string;
  label?: string;
    ...
    <div>
      <div
        ...
        className={`flex cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed p-10 text-center transition-colors ${
          dragOver ? "border-medical bg-mist" : "border-slate-300 bg-white hover:border-medical"
        }`}
      >
        <svg
          ...
          stroke="currentColor"
          strokeWidth={1.5}
          className="h-10 w-10 text-medical"
        >
          <path
            strokeLinecap="round"
